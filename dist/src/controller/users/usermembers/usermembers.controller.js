"use strict";
var __decorate = (this && this.__decorate) || function (decorators, target, key, desc) {
    var c = arguments.length, r = c < 3 ? target : desc === null ? desc = Object.getOwnPropertyDescriptor(target, key) : desc, d;
    if (typeof Reflect === "object" && typeof Reflect.decorate === "function") r = Reflect.decorate(decorators, target, key, desc);
    else for (var i = decorators.length - 1; i >= 0; i--) if (d = decorators[i]) r = (c < 3 ? d(r) : c > 3 ? d(target, key, r) : d(target, key)) || r;
    return c > 3 && r && Object.defineProperty(target, key, r), r;
};
var __metadata = (this && this.__metadata) || function (k, v) {
    if (typeof Reflect === "object" && typeof Reflect.metadata === "function") return Reflect.metadata(k, v);
};
var __param = (this && this.__param) || function (paramIndex, decorator) {
    return function (target, key) { decorator(target, key, paramIndex); }
};
Object.defineProperty(exports, "__esModule", { value: true });
exports.UsermembersController = void 0;
const common_1 = require("@nestjs/common");
const decorators_1 = require("@nestjs/common/decorators");
const usermembers_service_1 = require("../../../service/users/usermembers/usermembers.service");
const usermembers_dto_1 = require("./usermembers.dto");
let UsermembersController = class UsermembersController {
    constructor(userMembersService) {
        this.userMembersService = userMembersService;
    }
    findAllUserMembers() {
        return this.userMembersService.findAllUserMembers();
    }
    findAllJoinUserMembers() {
        return this.userMembersService.findAllJoinUserMembers();
    }
    findOneUserMembers(params) {
        return this.userMembersService.findOneUserMembers(params.id);
    }
    createUserMembers(body) {
        return this.userMembersService.createUserMembers(body);
    }
};
__decorate([
    (0, decorators_1.Get)(),
    __metadata("design:type", Function),
    __metadata("design:paramtypes", []),
    __metadata("design:returntype", void 0)
], UsermembersController.prototype, "findAllUserMembers", null);
__decorate([
    (0, decorators_1.Get)('join-all-usermembers'),
    __metadata("design:type", Function),
    __metadata("design:paramtypes", []),
    __metadata("design:returntype", void 0)
], UsermembersController.prototype, "findAllJoinUserMembers", null);
__decorate([
    (0, decorators_1.Get)(':id'),
    __param(0, (0, decorators_1.Param)()),
    __metadata("design:type", Function),
    __metadata("design:paramtypes", [Object]),
    __metadata("design:returntype", void 0)
], UsermembersController.prototype, "findOneUserMembers", null);
__decorate([
    (0, decorators_1.Post)(),
    __param(0, (0, decorators_1.Body)()),
    __metadata("design:type", Function),
    __metadata("design:paramtypes", [usermembers_dto_1.bodyUserMembersDto]),
    __metadata("design:returntype", void 0)
], UsermembersController.prototype, "createUserMembers", null);
UsermembersController = __decorate([
    (0, decorators_1.UsePipes)(new common_1.ValidationPipe()),
    (0, common_1.Controller)('usermembers'),
    __metadata("design:paramtypes", [usermembers_service_1.UsermembersService])
], UsermembersController);
exports.UsermembersController = UsermembersController;
//# sourceMappingURL=usermembers.controller.js.map